  ...
  return (
    <div className='px-10 mt-20 md:px-28 lg:px-36 xl:px-48'>
      <div className='flex justify-between items-center'>
        ...
        <RainbowButton disabled={selectedAssistant?.length==0||loading} onClick={OnClickContinue}>{loading&&<Loader2Icon className='animate-spin'/>}Continue</RainbowButton>
      </div>
      <div className='grid grid-cols-2 mid:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mt-5'>
        {AiAssistantsList.map((assistant, index) => (
          <BlurFade key={assistant.image} delay={0.25 + index * 0.05} inView>
            <div className='hover:border p-3 rounded-xl hover:scale-105 transition-all ease-in-out cursor-pointer relative' onClick={() => onSelect(assistant)} key={index}>
